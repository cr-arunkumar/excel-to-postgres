...
function inferType(values: any[]): string {
    ...
    const allIntegers = allNumbers && nonNullValues.every(v => Number.isInteger(Number(v)));
...
async function createTable(tableName: string, columns: ColumnInfo[]): Promise<void> {
...
async function importExcelToPostgres(filePath: string, tableName: string): Promise<void> {
    ...
    try {
        ...
        const transformer = new Transform({
            ...
            transform(row, encoding, callback) {
                const csvRow = columnNames.map(col => {
                    ...
                    if (columns.find(c => c.name === col)?.type === 'TIMESTAMP' && value) {
        ...
        const copyStream = sql.connection.stream(copyFrom.from(`COPY "${tableName}" (${columnNames.map(name => `"${name}"`).join(', ')}) FROM STDIN WITH (FORMAT CSV)`));
        
        // Pipe Excel data through transformer to COPY stream
        const excelStream = XLSX.stream.to_json(worksheet, { raw: true, rawNumbers: true });
